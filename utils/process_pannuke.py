...
data_dir = '../data/raw/folds/' # location to extracted folds
output_dir = '../data/processed/' # location to save op data 
...
for i, j in enumerate(folds):
    ...
    for k in trange(images.shape[0], desc='Writing files for {}'.format(j), total=images.shape[0]):
        ...
        # save file in op dir
        Image.fromarray(sem_mask).save(output_dir + '/{}/sem_masks/sem_{}_{}_{:05d}.jpg'.format(tissue_type, tissue_type, i+1, k)) 
        Image.fromarray(instances).save(output_dir +'/{}/inst_masks/inst_{}_{}_{:05d}.jpg'.format(tissue_type, tissue_type, i+1, k)) 
        Image.fromarray(raw_image).save(output_dir +'/{}/images/img_{}_{}_{:05d}.jpg'.format(tissue_type, tissue_type, i+1, k)) 
